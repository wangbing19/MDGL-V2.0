...
public class GlobalExceptionHandler {
	 ...
	 * 
	 * @ResponseBody public JsonResult doHandleShiroException( ShiroException e){
	 * e.printStackTrace(); JsonResult r=new JsonResult(); r.setState(0); if(e
	 * instanceof UnknownAccountException){ r.setMessage("用户不存在"); }else if(e
	 * instanceof LockedAccountException){ r.setMessage("账户已禁用"); }else if(e
	 * instanceof IncorrectCredentialsException){ r.setMessage("密码不正确"); }else if(e
	 * instanceof AuthorizationException){ r.setMessage("没有此操作权限"); }else{
